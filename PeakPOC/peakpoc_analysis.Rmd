...
count_tab_peakpoc <- count_tab_peakpoc[,-20]
#Load taxonomy table
...
sample_info_tab_peakpoc <- sample_info_tab_peakpoc[-4,]
...
Ps_peakpoc_merged <- merge_samples(Ps_peakpoc, "sample_ID")
...
#plot
plot_heatmap(Ps_peakpoc, method = "NMDS", distance = "bray", taxa.label = "Class", taxa.order = "Class", high = "red", low = "beige", na.value = "beige", max.label = ntaxa(Ps_peakpoc)) + labs(y = "ASVs")
```
...
plot_richness(Ps_peakpoc, measures = c("Chao1", "Shannon", "Simpson"), x = "sample_ID", color = "season") + geom_point(size = 4) + theme(axis.text.x = element_text(size = 8))
```
...
plot_richness(Ps_peakpoc, measures = c("Chao1", "Shannon", "Simpson"), x="collection_date_end", color="season") + labs(x = "Collection date end") + geom_point(size = 4) 
```
...
```{r}
# export a count table with right column names
count_peakpoc_correct_names <- as(otu_table(Ps_peakpoc_merged), "matrix")
merge_peakpoc <- merge(t(count_peakpoc_correct_names), tax_tab_peakpoc, by = 0, sort = FALSE)
